...
lemma bijective_imp_inversible {α : Type _} {β : Type _} (f : α → β) (h : is_bijective f) : ∃ (f_inv : β → α), (∀ (b : β), f (f_inv b) = b ∧ ∀ (a : α), f_inv (f a) = a) :=
by
...
lemma deterministic_function {α : Type _} {β : Type _} (f : α → β) : ∀ (a₁ a₂ : α), f a₁ ≠ f a₂ → a₁ ≠ a₂ :=
by
  intros a₁ a₂ h
  /- by_contra h2
  rw [h2] at h
