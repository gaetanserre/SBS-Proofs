...
variable [MeasurableSpace Ω] [MeasureSpace Ω] [MeasureSpace ℝ]
...
variable (is_integrable_H : ∀ (f : ℕ → Ω → ℝ), ∀ i ∈ range (d + 1), Integrable (f i) μ)

/--
We show that ⟪f, ϕ⟫ = 𝔼 x ∼ μ [∑ l ∈ range (d + 1), ((d_ln_π l x) * (f l x) + df l x)], where ϕ i x = ∫ y, (d_ln_π i y) * (k y x) + (dk y i x) ∂μ.
-/
lemma inner_product_eq_dKL (h1 : product_RKHS H H₀) (h2 : inner_product_H H) (f : ℕ → Ω → ℝ) (hf : f ∈ H) (df : ℕ → Ω → ℝ) : ⟪f, ϕ_ μ H₀ dk d_ln_π⟫ = ∫ x, ∑ l ∈ range (d + 1), ((d_ln_π l x) * (f l x) + df l x) ∂μ :=
by
  ...
  have invert_sum_integral : ∑ i ∈ range (d + 1), ∫ (y : Ω), (fun i y ↦ ⟪f i, fun x ↦ d_ln_π i y * s.k y x + dk y i x⟫) i y ∂μ = ∫ (y : Ω), ∑ i ∈ range (d + 1), (fun i y ↦ ⟪f i, fun x ↦ d_ln_π i y * s.k y x + dk y i x⟫) i y ∂μ := by {
    symm
...
-/
lemma bound_direction (h1 : product_RKHS H H₀) (h2 : inner_product_H H) (f : ℕ → Ω → ℝ) (hf : f ∈ H) (hfb : ‖f‖ = 1) (df : ℕ → Ω → ℝ) : ∫ x, ∑ l ∈ range (d + 1), ((d_ln_π l x) * (f l x) + df l x) ∂μ ≤ ‖ϕ_ μ H₀ dk d_ln_π‖ :=
by
  let ϕ := ϕ_ μ H₀ dk d_ln_π
