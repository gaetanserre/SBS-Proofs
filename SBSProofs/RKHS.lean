...
instance : Inner ℝ (product_RKHS H₀) where
  inner := λ f g ↦ ∑ i ∈ range (d + 1), ⟪f i, g i⟫

instance : Norm (product_RKHS H₀) where
  norm := λ f ↦ inner f f

instance : Norm (ℕ → H₀) where
  norm := λ (f : product_RKHS H₀) ↦ ‖f‖

variable [NormedAddCommGroup (ℕ → ℝ)]
/--
  The euclidean norm.
-/
def norm_H := ∀ (f : product_RKHS H₀), ∀x, (‖fun i ↦ (f i).1 x‖₊ : ℝ≥0∞) = sqrt (∑ i ∈ range (d + 1), ‖(f i).1 x‖₊^2)
